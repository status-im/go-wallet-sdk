...
func GoWSK_balance_multistandardfetcher_FetchBalances(ethClientHandle C.uintptr_t, chainIDC C.ulong, batchSizeC C.ulong, fetchConfigJSONCStr *C.char, errOut **C.char) *C.char {
	// Set up fetcher
	...
	ch := multistandardfetcher.FetchBalances(context.Background(), multicallAddress, multicallCaller, fetchConfig, int(batchSizeC))
