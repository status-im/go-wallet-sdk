...
func (f *FeeManager) estimatedTime(feeHistory *ethclient.FeeHistory, maxFeePerGas *big.Int) TransactionEstimation {
	fees := sortBigIntSlice(feeHistory.BaseFeePerGas)
	if len(fees) == 0 {
		return Unknown
	}
...
func estimatedTimeV2(feeHistory *ethclient.FeeHistory, txMaxFeePerGas *big.Int, txPriorityFee *big.Int, chainID uint64, rewardIndex int) uint {
	sortedBaseFees := sortBigIntSlice(feeHistory.BaseFeePerGas)
