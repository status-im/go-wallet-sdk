...
func (c *FakeClient) FeeHistory(ctx context.Context, blockCount uint64, lastBlock *big.Int, rewardPercentiles []float64) (*ethclient.FeeHistory, error) {
	...
	oldestBlockNumber := latestBlockNumber - blockCount + 1

	ret := &ethclient.FeeHistory{
		OldestBlock:   big.NewInt(int64(oldestBlockNumber)),
		BaseFeePerGas: make([]*big.Int, blockCount+1), // +1 for next block base fee
	...
	for i := uint64(0); i <= blockCount; i++ {
		...
		if startIdx-1+i < uint64(len(c.gasData.FeeHistory.BaseFeePerGas)) {
			ret.BaseFeePerGas[i] = big.NewInt(0).Set(c.gasData.FeeHistory.BaseFeePerGas[startIdx-1+i])
		} else {
			ret.BaseFeePerGas[i] = big.NewInt(0)
		}
	}
