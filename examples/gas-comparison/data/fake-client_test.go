...
func TestFakeClient_FeeHistory_Success(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if len(feeHistory.BaseFeePerGas) != tt.expectedBaseFees {
				t.Errorf("Expected %d base fees, got %d", tt.expectedBaseFees, len(feeHistory.BaseFeePerGas))
			}
...
func TestFakeClient_FeeHistory_SingleBlock(t *testing.T) {
	...
	if len(feeHistory.BaseFeePerGas) != 2 { // blockCount + 1
		t.Errorf("Expected 2 base fees, got %d", len(feeHistory.BaseFeePerGas))
	}
