...
func TestKeyStorePlain(t *testing.T) {
	...
	if !reflect.DeepEqual(k1.ExtendedKey, k2.ExtendedKey) {
		t.Fatal(err)
	}
}
...
func TestKeyStorePassphraseDecryptionFail(t *testing.T) {
	...
	if _, err = ks.GetKey(k1.Address, account.URL.Path, "bar"); err != keystore.ErrDecrypt {
		t.Fatalf("wrong error for invalid password\ngot %q\nwant %q", err, keystore.ErrDecrypt)
	}
}
...
func testDecryptV3(test KeyStoreTestV3, t *testing.T) {
	extBytes, _, err := decryptKeyV3(&test.Json, test.Password)
	if err != nil {
		t.Fatal(err)
	}
	privHex := hex.EncodeToString(extBytes)
