...
// TestImportExtendedKey tests the import functionality of a keystore.
func TestImportExtendedKey(t *testing.T) {
	t.Parallel()
	_, ks := tmpKeyStore(t)
	...
	key, err := extkeys.NewMaster(seed)
	if err != nil {
		t.Fatalf("failed to generate key: %v", key)
	}
	if _, err = ks.ImportExtendedKey(key, "old"); err != nil {
		t.Errorf("importing failed: %v", err)
	}
	if _, err = ks.ImportExtendedKey(key, "old"); err == nil {
		t.Errorf("importing same key twice succeeded")
	}
	if _, err = ks.ImportExtendedKey(key, "new"); err == nil {
		t.Errorf("importing same key twice succeeded")
	}
}
...
func TestImportExport(t *testing.T) {
	...
	json, err := ks.ExportExt(acc, "old", "new")
	if err != nil {
