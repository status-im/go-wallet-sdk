...
func TestTokensList_TokenMethods(t *testing.T) {
	...
	for _, token := range tokens {
		if _, ok := tokensByKey[tokenKey(token.ChainID, token.Address)]; ok {
			t.Fatal("duplicate token", tokenKey(token.ChainID, token.Address))
		}
		tokensByKey[tokenKey(token.ChainID, token.Address)] = struct{}{}
	...
	assert.Equal(t, len(tokenLists), 2)
	if tokenLists[0].Name == statusTokenList.Name {
		assert.Equal(t, len(statusTokenList.Tokens), len(tokenLists[0].Tokens))
		assert.Equal(t, len(uniswapTokenList.Tokens), len(tokenLists[1].Tokens))
	} else {
		assert.Equal(t, len(uniswapTokenList.Tokens), len(tokenLists[0].Tokens))
		assert.Equal(t, len(statusTokenList.Tokens), len(tokenLists[1].Tokens))
	}
