...
func TestNewDefaultCommunityTokenStore(t *testing.T) {
	store := NewDefaultCommunityTokenStore()
	assert.NotNil(t, store)
}

func TestDefaultCommunityTokenStore_GetAll(t *testing.T) {
	store := &defaultCommunityTokenStore{
		communityTokens: []Token{
			{
				Symbol:  "COMM1",
				Name:    "Community Token 1",
				ChainID: 1,
			},
			{
				Symbol:  "COMM2",
				Name:    "Community Token 2",
				ChainID: 137,
			},
		},
	}
	...
	assert.NoError(t, err)
	assert.Len(t, tokens, 2)
	assert.Equal(t, "COMM1", tokens[0].Symbol)
	assert.Equal(t, "COMM2", tokens[1].Symbol)
}
...
func TestDefaultConfig(t *testing.T) {
	...
	// Test default values
	assert.Equal(t, DefaultChains, config.Chains)
	...
	assert.NotNil(t, config.CommunityTokenStore)
...
func TestConfig_WithCommunityTokenStore(t *testing.T) {
	config := &Config{}
	store := &defaultCommunityTokenStore{}

	result := config.WithCommunityTokenStore(store)
	assert.Equal(t, config, result)
	assert.Equal(t, store, config.CommunityTokenStore)
}
