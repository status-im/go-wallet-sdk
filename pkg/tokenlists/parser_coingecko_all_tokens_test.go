...
func TestCoinGeckoAllTokensParser_Parse(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := parser.Parse(tt.raw, tt.sourceURL, timestamp)
...
func TestCoinGeckoAllTokensParser_Parse_InvalidJSON(t *testing.T) {
	...
	_, err := parser.Parse(raw, "https://example.com/invalid.json", time.Time{})
	assert.Error(t, err)
}
...
func TestCoinGeckoAllTokensParser_Parse_MissingFields(t *testing.T) {
	...
	raw := []byte(``)
	got, err := parser.Parse(raw, "https://example.com/missing-fields.json", time.Time{})
	assert.Error(t, err)
	assert.Nil(t, got)
