...
Native-token alias addresses (e.g., zkSync Era's `0x...800a` system alias) are not a builder concern: the manager normalizes alias lookups to the canonical zero-address native token (see `manager.Config.AdditionalAddressesForNativeToken`).
