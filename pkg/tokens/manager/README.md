...
- **⚡ Automatic Filtering**: Filtered tokens are excluded from the manager's *unique token collection* during building (e.g. `UniqueTokens`, `GetTokenByChainAddress`, `GetTokensByChain`, `GetTokensByKeys`)
- **📋 List Preservation**: Token lists returned by `TokenList` / `TokenLists` are stored and returned as loaded (unfiltered), even if all their tokens are filtered out of the unique token collection
