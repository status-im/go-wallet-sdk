...
- `char* GoWSK_balance_multistandardfetcher_FetchBalances(uintptr_t ethClientHandle, unsigned long chainID, unsigned long batchSize, char* fetchConfigJSON, char** errOut)` - Fetches balances across multiple token standards (Native ETH, ERC20, ERC721, ERC1155) using Multicall3 batched calls. The `fetchConfigJSON` parameter should be a JSON string with the configuration (see format below). Returns a JSON string with the results. Returns NULL on error. The returned string must be freed with `GoWSK_FreeCString`. If `errOut` is provided and an error occurs, it will contain an error message (must be freed with `GoWSK_FreeCString`).
